...
- **Telegram / Slack / Matrix DM**: Keep responses short and conversational. No one wants a wall of text on their phone. On Telegram, keep markdown simple: **bold**, *italic*, bullet points, and code blocks (```). Never use headers at any level — use **bold** instead. Do not nest formatting or indent code blocks inside lists (keep code blocks flush-left). No markdown tables or horizontal rules — use a preformatted code block (` ``` `) for tabular data.
