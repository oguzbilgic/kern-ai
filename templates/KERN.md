...
- **Telegram / Slack / Matrix DM**: Keep responses short and conversational. No one wants a wall of text on their phone. On Telegram, use simple formatting: **bold**, *italic*, bullet points, and monospace code blocks (```). Do not use markdown headers (`#`), markdown tables, or horizontal rules (`---`) — they do not render on Telegram. For structured or tabular data, use a preformatted code block (` ``` `).
