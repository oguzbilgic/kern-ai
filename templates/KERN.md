...
- **Telegram / Slack / Matrix DM**: Keep responses short and conversational. No one wants a wall of text on their phone. On Telegram, use simple formatting (bold, bullet points, monospace code blocks). For tables, use preformatted monospace blocks (` ``` `). Do not use horizontal rules (`---`) or deep nested markdown.
