...
### Features
- **Chat commands: unified in-chat commands with bang (`!`) and slash (`/`) prefixes** ([#369](https://github.com/oguzbilgic/kern-ai/pull/369), [#376](https://github.com/oguzbilgic/kern-ai/issues/376), [#393](https://github.com/oguzbilgic/kern-ai/issues/393)) — renames "slash commands" to **Chat Commands**, supporting both `!` and `/` prefixes interchangeably (`!status` or `/status`, `!help` or `/help`). The `!` prefix prevents native client-side interception in chat apps like Matrix, Slack, and Discord. Commands are intercepted at the message queue layer, execute instantly at zero LLM token cost, and broadcast output cleanly back to the calling interface.
...
- **`recall-repair` offline tool** ([#384](https://github.com/oguzbilgic/kern-ai/pull/384), [#385](https://github.com/oguzbilgic/kern-ai/pull/385)) — recovery tool for recall vector indexes with orphaned chunks. Pure SQLite (zero LLM calls, zero API credentials). Prunes orphaned chunks and rewinds the index scan cursor so the agent's native background indexer cleanly re-indexes missing messages on next start or turn. Accessible offline via `kern scripts recall-repair <recall.db>`. Dry-run by default; zero-op when index is already healthy (0 changes, 0 writes).
- **`recall-health` diagnostic tool and chat command** ([#370](https://github.com/oguzbilgic/kern-ai/pull/370), [#371](https://github.com/oguzbilgic/kern-ai/pull/371), [#392](https://github.com/oguzbilgic/kern-ai/pull/392)) — diagnostic tool for `recall.db`. Checks message lag, chunk size distribution, batch blockers, surrogate pair integrity, vector table sync, and stalled pipeline blockers with an itemized 0–100 health score. Accessible live in chat via `/recall-health` or `!recall-health` (bypassing the LLM queue for instant read-only diagnostics), or offline via `kern scripts recall-health <recall.db>`.
- **`segment-health` chat command** ([#371](https://github.com/oguzbilgic/kern-ai/pull/371)) — live chat command (`/segment-health` or `!segment-health`) to inspect the semantic summary tree directly from chat, bypassing the LLM queue to report levels, overlaps, gaps, and injection token waste.
