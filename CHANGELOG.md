...
### Fixes
- **`.kern/.env` now overrides inherited environment variables** ([#306](https://github.com/oguzbilgic/kern-ai/pull/306)) — dotenv previously kept existing `process.env` values, so an agent started from another agent's process (or any shell with provider keys exported) silently inherited the parent's `OPENAI_API_KEY`, `OPENAI_BASE_URL`, etc. instead of its own. The agent's `.kern/.env` is now authoritative. Note the precedence flip: a value set in both the process environment and `.kern/.env` now resolves to the file. In Docker, pass secrets via `-e` *or* the mounted `.env`, not both — the file wins. (`KERN_*` config overrides are unaffected.)
