...
### Improvements
- **Pin `unpdf` to exact 1.6.2** ([#285](https://github.com/oguzbilgic/kern-ai/issues/285)) — unpdf 1.6.0 broke the `pdf` tool on Node 22 (`Promise.try` usage); upstream fixed it with a polyfill in 1.6.1. Pinned to the verified-good 1.6.2 so future unpdf releases can't silently break fresh installs again. Bumped deliberately from now on.
