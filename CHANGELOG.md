...
### Features
- **IRC interface** — talk to your agent from any IRC client, on any network, including a self-hosted one. Set `irc` in config (or `IRC_URL` in env) to a connection URL — `ircs://myagent@irc.example.com:6697/#homelab` — and the agent joins and listens. Zero new dependencies: raw TCP/TLS with IRCv3 capability negotiation. DMs are gated by pairing; channels stay open so agents can follow ongoing context and collaborate. Because IRC nicks are trivially spoofable, identity is keyed on the server-verified IRCv3 `account-tag` rather than the nick — an unauthenticated sender is marked `~nick` and never auto-paired. Multiple networks are namespaced by host so accounts can't collide. Outbound markdown becomes IRC formatting codes, wrapped to the 512-byte line limit and rate-limited to avoid flood kicks. Turn failures are reported back in the channel instead of being swallowed — silence is indistinguishable from being ignored — and every outbound line is logged, so "did it send?" is answerable from the log.
...
- **Built-in `irc` tool & `irc-setup` skill** — the agent can probe IRC servers, register its own NickServ account in-band, configure its connection URL, join or part channels, check channel user lists, and verify user identity via WHOIS. No raw socket scripting needed: ask the agent to connect to an IRC server and it handles the registration and config flow autonomously.
...
### Behaviour Changes
- **Room etiquette & group chat discipline** — agents in shared rooms (Slack channels, Matrix group rooms, IRC channels, group chats) now receive a dedicated room etiquette guide in their prompt. Instead of feeling obligated to chime in on every message or fill silence with polite summaries, agents are instructed to speak only when explicitly addressed or participating in an ongoing exchange, and to exit cleanly with `NO_REPLY` as soon as the question is answered or humans begin talking among themselves. When multiple agents share a room, they no longer get trapped in reciprocal acknowledgement loops.
