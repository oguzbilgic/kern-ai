...
### Improvements
- **Refreshed model defaults** — default model is now `anthropic/claude-opus-4.8` (was 4.7); init fallback lists updated across all providers (OpenAI list was still GPT-4o/GPT-4.1/o3 — now GPT-5.5/5.5 Pro/5.4 Mini; added Claude Fable 5; Ollama list refreshed to current models); Anthropic vision fallback for media digest updated from a dated 2025 snapshot ID to `claude-sonnet-4-6`. Summary model defaults intentionally unchanged — `gpt-4.1-mini`/`claude-haiku-4.5` are cheap and non-thinking, which summary calls require.
