...
### Improvements
- **Matrix DM resolution in `sendToUser`** — `MatrixInterface.sendToUser` now resolves direct chats when given a Matrix user ID (`@user:server`). It checks `m.direct` account data for an existing DM room, or creates a new direct chat (`is_direct: true`, `trusted_private_chat`), updates `m.direct`, and routes messages directly to the 1:1 conversation rather than requiring an explicit room ID or leaking proactive messages into group rooms.
- **Switch Matrix markdown parser to `marked` for full GFM support** — replaces manual regex markdown parsing in `mdToMatrixHtml` with `marked`. Resolves nested and ordered list formatting issues, supports full GFM tables with alignment, blockquotes, syntax-highlighted code blocks, and compliant HTML output without regex edge cases.
- **Matrix table rendering support in `mdToMatrixHtml`** — markdown tables (`| col | col |`) with header alignments (`:---`, `:---:`, `---:`) are now parsed and converted into semantic HTML tables (`<table>`, `<tr>`, `<th>`, `<td>`), with inline formatting (bold, italic, links, code) rendered inside cells for Matrix clients.
- **Clean up redundant line breaks around Matrix block elements** — strips unnecessary `<br />` tags immediately preceding and following block-level elements (`<h1>`-`<h6>`, `<ul>`, `<ol>`, `<pre>`, `<blockquote>`, `<hr>`, `<table>`) in `mdToMatrixHtml`. In Matrix clients like Element and Cinny, block tags have native margins; stripping the surrounding line breaks eliminates excessive vertical whitespace between paragraphs and headers.
