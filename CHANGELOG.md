...
### Fixes
- **Summary tree tiling invariant** ([#364](https://github.com/oguzbilgic/kern-ai/issues/364)) — each level of the semantic segment tree now tiles the indexed history exactly once. Four root causes of overlapping and mis-parented segments closed:
  - `indexSession` resumes strictly *after* `last_segmented_msg` instead of re-including it, so chunk boundaries no longer produce a 1-message overlap (1,661 of them on one agent).
  - `rollUpLevels` groups only *contiguous* orphans (legacy 1-msg fenceposts tolerated); a discontinuity ends the group. Short runs sitting in a hole between existing parents (or at the level start) roll up as-is — folded into the neighbouring full group when there is one — so stragglers cannot pile up. Previously orphans were batched in tens by position alone, producing parents that spanned weeks of siblings they never summarized (one covered 108k messages).
  - New L0 segments that overlap existing same-level coverage are rejected at insert (the `UNIQUE` constraint only caught exact duplicates), with a warning; the cursor still advances so the stretch is never re-embedded. A missing `segment_state` row with an existing tree fast-forwards the cursor to the tree instead of re-segmenting from message 0.
  - An embedding-dimension change no longer resets `segment_state`. Segments and summaries do not depend on the embedding model; the reset re-embedded the whole history and laid a second, shifted tiling next to the first on every provider switch — the origin of most fleet overlaps.
  - `composeHistory` drops a root whose range is fully inside another root's (safety net; zero residual after the above).
  Pure grouping step exported as `planRollupGroups()`; 12 new tests including end-to-end `indexSession` fixtures and a prune → rollup → 100/100 scenario.
