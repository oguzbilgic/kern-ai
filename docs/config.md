...
| `mentionsOnly` | `true` | In group chats and channels, only start a turn when the agent is addressed — an @mention or a reply to one of its own messages. Unaddressed messages are still observed and folded into the next addressed turn as context, so the agent keeps the room without speaking in it. DMs, TUI, web and heartbeats are never gated. Set to `false` for the old behavior, where every group message ran a turn and the model decided whether to answer. See [Interfaces § Mentions-only](interfaces.md#mentions-only). |
