...
Changing this on an agent that already has memory is only safe when the new model has a different vector width. A different-width change recreates the vector tables and re-vectorizes the existing chunks. kern tracks the width, not which model produced the vectors, so a same-width change (`gemini-embedding-001` to another 3072-dimension model) leaves the old vectors in place and searches them with vectors from a different embedding space.
