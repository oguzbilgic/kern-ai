...
| `subAgentModel` | `""` | Model for spawned sub-agents, on the parent's provider. Empty = inherit the parent's `model`. Sub-agents are read-only and bounded, so a cheaper model (e.g. `anthropic/claude-haiku-4.5`, `openai/gpt-4.1-mini`) usually suffices — in heavy research fan-out they can account for most of the token volume. Individual `spawn` calls can override per-child. |
