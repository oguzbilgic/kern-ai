...
| `audioModel` | `""` | Audio-capable model for the `audio` tool and voice-message transcription at ingest. Fallback chain: `audioModel` → agent model → provider default (`google/gemini-3.1-flash-lite` on OpenRouter, `gpt-audio-mini` on OpenAI; none for anthropic/ollama — set this field explicitly). |
