...
2. **Single-User Environment (`~/.kern/config.json`)**:
   ...
   - Supervised by `kern start` (detached daemon with `.kern/agent.pid`) or run in the foreground with `kern run` (Docker, custom supervisors). No systemd units are created.
